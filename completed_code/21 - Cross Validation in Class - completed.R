...
# cars_rec %>% prep() %>% bake(new = cars_training) %>% glimpse()
...
# perform the fit
set.seed(42)
cars_folds <- vfold_cv(cars_training, strata = sellingprice_log, repeats = 2)


cars_folds

cars_cv_result <- cars_wkfl %>% 
  fit_resamples(resamples = cars_folds)

cars_cv_result %>% collect_metrics(summarize = F)
